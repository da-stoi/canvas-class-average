...
export async function getGradeHistory(course: number): Promise<GradeHistory[]> {
  ...
  const cca = JSON.parse(localStorage.getItem('cca') || '{}');
  ...
  const sortedGradeHistory = parsedGradeHistory.sort(
    (a: GradeHistory, b: GradeHistory) => {
...
export async function appendGradeHistory(
  ...
  if (
    ...
    cca.gradeHistory[course][cca.gradeHistory[course].length - 1].average ===
      newGradeHistory.average &&
    cca.gradeHistory[course][cca.gradeHistory[course].length - 1].total ===
      newGradeHistory.total
  ) {
  ...
  if (!cca.gradeHistory || !cca.gradeHistory[course]) {
    ...
    let updatedCca = {
      ...
      gradeHistory: {
        ...cca.gradeHistory,
        [course]: [newGradeHistory],
      },
    };

    // Set updated cca data
    localStorage.setItem('cca', JSON.stringify(updatedCca));
  ...
  let updatedCca = {
    ...cca,
    gradeHistory: {
      [course]: [...cca.gradeHistory[course], newGradeHistory],
    },
  };
...
export async function setSettings(settings: Settings): Promise<void> {
  // Get existing cca data
  let cca = JSON.parse(localStorage.getItem('cca') || '{}');

  // Update cca data
  cca.settings = settings;

  // Set updated cca data
  localStorage.setItem('cca', JSON.stringify(cca));
}
