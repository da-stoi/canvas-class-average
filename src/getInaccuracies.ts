...
function getInaccuracies(
  ...
  if (Object.keys(weightGroups).length <= 1) {
    inaccuracies.push(
      'Your instructor has not published weight groups. This may cause inaccuracies.'
    );
  }

  // Check if there are any assignments that are dropped
  const droppedAssignments = assignments.filter(
    (assignment) => assignment.dropped
  );
  if (droppedAssignments.length > 0) {
    inaccuracies.push(
      `You have ${droppedAssignments.length} dropped assignment${
        droppedAssignments.length > 1 ? 's' : ''
  ...
  const missingAssignments = assignments.filter(
    (assignment) => assignment.isMissing
  );
  if (missingAssignments.length > 0) {
    inaccuracies.push(
      `You are missing ${missingAssignments.length} assignment${
        missingAssignments.length > 1 ? 's' : ''
      }. This will most likely cause inaccuracies.`
    );
  }
  ...
  Object.keys(weightGroups).forEach((weightGroup) => {
    if (
      !gradesByWeightGroup[weightGroup]?.average &&
      weightGroup !== 'total' &&
      weightGroups[weightGroup] > 0
    ) {
  ...
  if (weightGroupsWithoutAverage > 0) {
    inaccuracies.push(
      `Average may be off by up to ${(totalWeightWithoutAverage * 100).toFixed(
        0
      )}% because ${weightGroupsWithoutAverage} weight group${
        weightGroupsWithoutAverage === 1 ? ' does' : 's do'
      } not have an average. Check the Average column in the Weight Groups table for more accurate information.`
    );
  }
...
export function displayInaccuracies(
  assignments: Assignment[],
  weightGroups: WeightGroups
) {
